...
function Readability(doc, options) {
	...
	if (this._debug) {
		const logNode = (node) => {
			...
			const attrPairs = Array.from(node.attributes || [], (attr) => `${attr.name}="${attr.value}"`).join(' ');
...
Readability.prototype = {
	...
	REGEXPS: {
		...
		positive: /article|body|content|entry|hentry|h-entry|main|page|pagination|post|text|blog|story|articleBody/i,
	...
	_getArticleTitle() {
		...
		if (new RegExp(`\\s[${titleSeparators}]\\s`).test(curTitle)) {
			...
			var match = this._someNode(headings, (heading) => heading.textContent.trim() === trimmedTitle);
	...
	_grabArticle(page) {
		...
		while (true) {
			...
			this._forEachNode(elementsToScore, function(elementToScore) {
				...
				contentScore += Math.min((innerText.length / 100) ** 1.25, 3);
		...
		this._forEachNode(
			...
			function(elem) {
				...
				if (!elem.src || (elem.src && this.REGEXPS.b64DataUrl.test(elem.src))) {
					var lazyAttrs = ['data-src', 'data-original', 'data-lazy-src', 'data-lazy', 'data-source', 'data-bg'];
	...
	_cleanConditionally(e, tag) {
		...
		this._removeNodes(this._getAllNodesWithTag(e, [tag]), function(node) {
			...
			if (
				this.REGEXPS.carousel.test(node.className || '')
			...
			if (this._getCharCount(node, ',') < 10) {
				...
				const shouldRemoveNode = () => {
					...
					if (totalHeaders >= 3 && linkedHeaders / totalHeaders > 0.5) {
						errs.push(
							`High linked header density suggests hub page (linkedHeaders=${linkedHeaders}, totalHeaders=${totalHeaders})`,
						);
					}
