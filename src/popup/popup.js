...
const countUtils = globalThis.snipSnipCountUtils;
...
function normalizeDefaultSendToTarget(
	value,
	customTargets = normalizeSendToCustomTargets(currentOptions?.sendToCustomTargets),
) {
	const optionsStateApi = getOptionsStateApi();
...
function normalizeSendToMaxUrlLength(
	value,
...
function buildMarkedRenderer({ renderImages = false } = {}) {
	...
	renderer.image = function({ href, title, text }) {
		...
		if (!renderImages) {
			return `<span class="preview-image-placeholder">[image: <a href="${
				escapeHtml(safeSrc)
			}" target="_blank" rel="noopener noreferrer"${titleAttr}>${alt || escapeHtml(safeSrc)}</a>]</span>`;
		}
	...
	return browser.scripting.executeScript({
		...
		func: async ({ htmlDocument: nextHtmlDocument, kind: nextKind }) => {
			const cleanupExisting = () => {
				document.getElementById('__snipsnip-print-frame__')?.remove();
			};

			cleanupExisting();

			const iframe = document.createElement('iframe');
			iframe.id = '__snipsnip-print-frame__';
...
async function handlePrintExport(
	kind = 'print',
	{ markdown = getEditorValue(), title = getCurrentExportTitle() } = {},
) {
	closeSplitDropdown();
...
function loadNotificationHostDeferred() {
	...
	notificationHostLoadPromise = loadScriptOnce('../notifications/notification-host.js', 'notification-host-script')
		.catch((error) => {
...
function resolveEditorTheme(
	...
	isDark,
	specialTheme = 'none',
	colorBlindTheme = currentOptions?.colorBlindTheme,
) {
...
async function showShortcutsModal() {
	...
	document.querySelectorAll('body > :not(#shortcutsModal)').forEach(el => {
		el.inert = true;
	});
	...
	try {
		...
		const fragment = globalThis.snipSnipPopupShortcuts.buildShortcutsFragment(document, commands);
...
function getPopupBatchUtilsApi() {
	return globalThis.snipSnipPopupBatchUtils || null;
}
...
function validateAndPreviewUrls() {
	...
	if (validCount === 0) {
		...
		urlValidation.textContent = `${validCount} valid URL${validCount !== 1 ? 's' : ''}, ${invalidCount} invalid line${
			invalidCount !== 1 ? 's' : ''
		}`;
	...
	while (Date.now() - start < maxWaitMs) {
		try {
			...
			if (snapshot) {
				...
				const richContentStable = snapshot.textLength >= 900
					&& stablePolls >= 2
					&& elapsed >= 2000;

				const shortPageStable = snapshot.textLength >= 120
...
async function clipTabWithRetry(tab, maxAttempts = 2) {
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		...
		console.log(
			`[Batch] Tab ${tab.id} attempt ${attempt}/${maxAttempts}: markdownLength=${markdownLength}, incomplete=${incomplete}`,
		);
...
async function handleBatchConversion(e) {
	...
	// Keep inline mode for e2e tests by setting window.__SNIPSNIP_FORCE_INLINE_BATCH__ = true.
	if (!window.__SNIPSNIP_FORCE_INLINE_BATCH__) {
...
async function handleSendToAction(
	targetId = currentOptions?.defaultSendToTarget,
	{ selectionOnly = false, triggerButton = null } = {},
) {
	closeSplitDropdown();
	...
	const { launchUrl, fallbackUrl, requiresClipboardFallback } = buildAssistantLaunchUrls(
		target,
		content,
		currentOptions,
	);
...
async function sendToObsidian(e) {
	...
	try {
		...
		const markdown = snipSnipObsidian.prepareMarkdownForObsidian(
			getEditorValue(),
			sourceImageMap || {},
		);
