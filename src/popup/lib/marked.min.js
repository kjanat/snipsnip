...
(function(g, f) {
	...
	var m = {
		...
		oe = h(ie).replace(/bull/g, F).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(
			/fences/g,
			/ {0,3}(?:`{3,}|~{3,})/,
		).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/)
			.replace(/\|table/g, '').getRegex(),
		Me = h(ie).replace(/bull/g, F).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(
			/fences/g,
			/ {0,3}(?:`{3,}|~{3,})/,
		).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/)
			.replace(/table/g, / {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),
		...
		Ce = h(
			...
			'i',
		).replace('comment', K).replace('tag', v).replace(
			'attribute',
			/ +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/,
		).getRegex(),
		le = h(Q).replace('hr', O).replace('heading', ' {0,3}#{1,6}(?:\\s|$)').replace('|lheading', '').replace(
			'|table',
			'',
		...
		).replace('hr', O).replace('heading', ' {0,3}#{1,6}(?:\\s|$)').replace('blockquote', ' {0,3}>').replace(
			'code',
			'(?: {4}| {0,3}	)[^\\n]',
		...
		Oe = {
			...
			lheading: Me,
			table: re,
			paragraph: h(Q).replace('hr', O).replace('heading', ' {0,3}#{1,6}(?:\\s|$)').replace('|lheading', '').replace(
				'table',
				re,
			).replace('blockquote', ' {0,3}>').replace('fences', ' {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n').replace(
				'list',
		...
		Be = {
			...
			fences: I,
			lheading: /^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,
			paragraph: h(Q).replace('hr', O).replace(
				'heading',
				` *#{1,6} *[^
]`,
			).replace('lheading', oe).replace('|table', '').replace('blockquote', ' {0,3}>').replace('|fences', '').replace(
				'|list',
				'',
		...
		Ke = h(
			...
			'gu',
		).replace(/notPunctSpace/g, ce).replace(/punctSpace/g, W).replace(/punct/g, D).getRegex(),
		Xe = h(/\\(punct)/, 'gu').replace(/punct/g, D).getRegex(),
		We = h(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace('scheme', /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace(
			'email',
		...
		).replace('comment', Je).replace(
			'attribute',
			/\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/,
		).getRegex(),
		q = /(?:\[(?:\\.|[^\[\]\\])*\]|\\.|`[^`]*`|[^\[\]\\`])*?/,
		Ye = h(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace('label', q).replace(
			'href',
			/<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/,
		...
		j = {
			...
			emStrongLDelim: Fe,
			url: h(/^((?:ftp|https?):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/, 'i').replace(
				'email',
	...
	var st = { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }, fe = l => st[l];
	function R(l, e) {
		if (e) { if (m.escapeTest.test(l)) return l.replace(m.escapeReplace, fe); }
		...
		code(e) {
			...
			if (t) {
				...
				return {
					...
					text: this.options.pedantic ? n : A(
						n,
						`
`,
					),
...
`,
							1,
						)[0].replace(this.rules.other.listReplaceTabs, Z => ' '.repeat(3 * Z.length)),
						g = e.split(
							`
`,
							1,
...
` + C.slice(f);
								...
								if (
									T || d.replace(this.rules.other.tabCharGlobal, '    ').search(this.rules.other.nonSpaceChar) >= 4
									|| ne.test(d) || se.test(d) || te.test(d)
								) break;
								u += `
` + g;
							}
...
`
		...
		tag(e) {
			...
			if (t) {
				return !this.lexer.state.inLink && this.rules.other.startATag.test(t[0])
					...
					!this.lexer.state.inRawBlock && this.rules.other.startPreScriptTag.test(t[0])
						...
						: this.lexer.state.inRawBlock && this.rules.other.endPreScriptTag.test(t[0])
...
` + s.text,
		...
		inlineTokens(e, t = []) {
				...
				if (o.length > 0) {
					for (
						...
						o.includes(s[0].slice(s[0].lastIndexOf('[') + 1, -1))
							&& (n = n.slice(0, s.index) + '[' + 'a'.repeat(s[0].length - 2) + ']'
			...
			for (; (s = this.tokenizer.rules.inline.blockSkip.exec(n)) != null;) {
				n = n.slice(0, s.index) + '['
					+ 'a'.repeat(s[0].length - 2) + ']' + n.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);
			}
			let i = !1, r = '';
			for (; e;) {
				i || (r = ''), i = !1;
...
`;
			return s
				? '<pre><code class="language-' + R(s) + '">' + (n ? i : R(i, !0)) + `</code></pre>
`
	...
	var E = class {
						...
						n.extensions && (n.extensions.forEach(i => {
							...
							if ('tokenizer' in i) {
								...
								r ? r.unshift(i.tokenizer) : t[i.level] = [i.tokenizer],
									i.start && (i.level === 'block'
										? t.startBlock ? t.startBlock.push(i.start) : t.startBlock = [i.start]
										: i.level === 'inline'
		...
		parseMarkdown(e) {
			return (n, s) => {
				...
				if (r.async) {
					return Promise.resolve(r.hooks ? r.hooks.preprocess(n) : n).then(p => a(p, r)).then(p =>
						r.hooks ? r.hooks.processAllTokens(p) : p
					).then(p => r.walkTokens ? Promise.all(this.walkTokens(p, r.walkTokens)).then(() => p) : p).then(p => c(p, r))
						.then(p => r.hooks ? r.hooks.postprocess(p) : p).catch(o);
				}
				try {
