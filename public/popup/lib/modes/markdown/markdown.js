...
((mod) => {
	...
	CodeMirror.defineMode('markdown', (cmCfg, modeCfg) => {
		...
		function blockNormal(stream, state) {
			...
			var match = null;
			if (
				state.indentationDiff >= 4 && (prevLineIsIndentedCode || state.prevLine.fencedCodeEnd
				...
				firstTokenOnLine && state.indentation <= maxNonCodeIndentation && (match = stream.match(atxHeaderRE))
				&& match[1].length <= 6
			) {
				state.quote = 0;
				...
				!isHr && !state.setext && firstTokenOnLine && state.indentation <= maxNonCodeIndentation
		...
		function local(stream, state) {
			...
			if (
				state.fencedEndRE && state.indentation <= maxFencedEndInd && (hasExitedList || stream.match(state.fencedEndRE))
			) {
				if (modeCfg.highlightFormatting) state.formatting = 'code-block';
				var returnType;
